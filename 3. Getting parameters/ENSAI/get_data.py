...
R_n = np.loadtxt( 'PS_4_real.txt')
I_n = np.loadtxt( 'PS_4_imag.txt')
...
def make_real_noise(Fmap):
    Npix = Fmap.shape[0];
    Npix_2 = int(Npix/2)
    Npix_2p1 = int(Npix/2 + 1)
    Npix_2p2 = int(Npix/2 + 2)
    Npix_2m1 = int(Npix/2 - 1)
...
def read_data_batch( X , Y , mag , max_file_num , train_or_test, seed = 2):
    batch_size = len(X)
    ...
    for i in range(batch_size):
        ...
        while True:
            ...
            while np.min(ARCS)==1 or np.max(ARCS)<0.4:
               ...
               #print('pick_folder' + str(pick_folder))
               # print('arc_filename' + str(arc_filename))
               # print('lens_filename' + str(lens_filename))
                ...
                ARCS = np.array(Image.open(arc_filename),dtype='float32')
                ARCS = (ARCS/np.max(ARCS)).reshape(numpix_side*numpix_side,)
                #print(np.max(ARCS))
                LENS = np.array(Image.open(lens_filename),dtype='float32')
                LENS = (LENS/np.max(LENS)).reshape(numpix_side*numpix_side,)
        ...
        #im_telescope = im_telescope * mask
